...
export function getCarLocalImages(brand, modelName) {
  try {
    ...
    // Find matching brand key in manifest (robust contains match)
    const matchedBrandKey = Object.keys(carImagesManifest).find(
      (key) => {
        const keyNorm = normalize(key);
        return keyNorm.includes(targetBrandNorm) || targetBrandNorm.includes(keyNorm);
      }
    );
...
export function enrichCarWithLocalImage(car) {
  ...
  // Prefer database saved vehicle_image if it exists and is a database asset (ignoring the Unsplash placeholder)
  if (car.vehicle_image && 
      !car.vehicle_image.startsWith('/EV Cars/') && 
      !car.vehicle_image.includes('images.unsplash.com/photo-1593941707882-a5bba14938c7')) {
    return {
      ...car,
      vehicle_image: getImageUrl(car.vehicle_image)
    };
  }
