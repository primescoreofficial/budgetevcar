...
export function getCarUrl(car) {
  ...
  const slug = `${brand}-${model}`.replace(/(^-|-$)+/g, '');
  return `/cars/${slug}${car.serial_no ? `?v=${car.serial_no}` : ''}`;
}

export async function getCarBySlug(slug, variantSerialNo = null) {
  const cars = await getAllCars();

  if (variantSerialNo) {
    const serial = parseInt(variantSerialNo, 10);
    const carBySerial = cars.find(car => car.serial_no === serial);
    if (carBySerial) {
      const brand = (carBySerial.brand || '').toLowerCase().replace(/[^a-z0-9]+/g, '-');
      const model = (carBySerial.model_name || carBySerial.detailed_name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-');
