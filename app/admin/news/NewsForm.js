...
export default function NewsForm({ newsId = null }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [fetching, setFetching] = useState(!!newsId);
  ...
  // Upload handler for news bucket
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      // Upload directly to 'news' bucket
      const { data, error: uploadError } = await supabase.storage
        .from('news')
        .upload(filePath, file);

      if (uploadError) {
        throw new Error(uploadError.message + '. Please ensure a public "news" bucket exists in Supabase Storage.');
      }
    ...
    try {
      ...
      try {
        await supabase.from('activity_logs').insert({
          ...
          action: newsId ? 'News Edited' : 'News Added',
          details: `Saved news article "${formData.title}"`
        });
      } catch (logErr) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      {/* Page Header */}
      <PageHeader 
        title={newsId ? 'Edit News Article' : 'Create News Article'} 
        description="Write and publish EV industry news, alerts, and market releases."
        action={backAction}
      />
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="space-y-4">
            ...
            <Input
              ...
              placeholder="e.g. Tata Motors Launches Punch EV Under 11 Lakhs"
              required
            />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                ...
                value={formData.slug}
                onChange={handleChange}
                placeholder="e.g. tata-motors-launches-punch-ev"
                required
              />
              <Input
                label="Category"
                ...
                onChange={handleChange}
                placeholder="e.g. Launch, Subsidy, Battery Tech"
              />
            </div>
            ...
            <TextArea
              ...
              onChange={handleChange}
              placeholder="A brief introduction of the article (appears in news feeds)..."
              className="h-20"
            />
            ...
            <TextArea
              ...
              onChange={handleChange}
              placeholder="# Introduce your news article here..."
          ...
          <Card className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="e.g. launch, subsidy"
              />
              <Button
                type="button"
                variant="secondary"
