...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `EV Guides on ${categoryName} — BudgetEV`,
    description: `Read all our expert electric vehicle articles and guides categorized under ${categoryName}.`
  };
}
...
export default async function BlogCategoryPage({ params }) {
  const { slug } = await params;
  const categoryName = slug.replace(/-/g, ' ');
  const posts = await getAllPosts('blogs', { category: slug });
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-5 pb-24">
        ...
        <div className="mb-10 pb-6 border-b border-slate-200/50">
          ...
          <p className="text-slate-500 text-sm font-semibold mt-1.5 max-w-xl">
            Browse all EV guides and analysis posts categorized under {categoryName}.
          </p>
        </div>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-20 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm">
            ...
            <p className="text-slate-400 text-sm font-semibold mb-6">
              We couldn't find any articles under the "{categoryName}" category.
            </p>
